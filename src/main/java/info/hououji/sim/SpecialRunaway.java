...
//			System.out.println("skip:" + csv.getCode()) ;
		...
		for(int i=0; i<=period; i++) {
			...
			double checkIsNormal = (csv.get(i, CSV.ADJ_CLOSE) / csv.get(i+1, CSV.ADJ_CLOSE)) 
					/ (csv.get(i, CSV.CLOSE) / csv.get(i+1, CSV.CLOSE)) ;
			checkIsNormal = Math.abs(checkIsNormal - 1) ;

			double stockGap = (newLow - oldHigh) / oldHigh ;
//			System.out.println("date:" + csv.getDate(i)+
//					"," + newLow +","+ oldHigh +
//					"," + (newLow - oldHigh) / oldHigh ) ;
			
			if(stockGap > 3) continue ;// too large, it must be some problem
